...
df=st.cache(pd.read_csv)('FinancialFraud_SampleData.xlsx')
...
outlier_percentage=(df.isFraud.value_counts()[1]/df.isFraud.value_counts()[0])*100
...
n_top_features = st.sidebar.slider('Number of top features', min_value=3, max_value=7)
...
if st.sidebar.checkbox('Financial Fraud Detection System'):
    step=st.number_input("Enter the step")
    amount = st.number_input("Enter the amount") 
    oldbalanceOrg = st.number_input("Enter the Old Balance") 
    newbalanceOrg = st.number_input("Enter the New Balance")
    oldbalanceDest=st.number_input("Enter the Old Balance Dest")
    newbalanceDest=st.number_input("Enter the New Balance Dest")
    type=st.selectbox('type',(0,1,2,3,4))
    st.write("0= CASH_IN")
    st.write("1= CASH_OUT")
    st.write("2= DEBIT")
    st.write("3= PAYMENT")
    ...
    def prediction(step,type,amount,oldbalanceOrg,newbalanceOrg,oldbalanceDest,newbalanceDest):
        prediction = model.predict( [[step,type,amount,oldbalanceOrg,newbalanceOrg,oldbalanceDest,newbalanceDest]])
        if prediction == 0:
            pred = 'Fraud'
        else:
            pred = 'Genine'
        return pred
    result =""
    new_input=[[step,type,amount,oldbalanceOrg,newbalanceOrg,oldbalanceDest,newbalanceDest]]
    if st.button("Predict"): 
        result = model.predict(new_input) 
        st.success('The transation is {}'.format(result))
